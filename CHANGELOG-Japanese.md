...
- サマリーの `Dates with most total detections` がすべてのレベルで `n/a` と表示され、`first_event_time`/`last_event_time` が設定されず、さらに **Sigma の相関（correlation）ルールが一切発火しない** 問題を修正した。原因は、イベントのタイムスタンプを取得する際に出力プロファイルのフィールド指定をそのまま `Event::get` に渡していたことである。プロファイルの指定は先頭にドットが付き、`|` 区切りのフォールバックリストになることもある（AWS は `.eventTime`、Azure/M365 は `.time|.eventTimestamp|.CreationTime`）一方で、`Event::get` は「ドットなし」のフィールド名を受け取り `.` をネストの区切りとして解釈するため、`event.get(".eventTime")` は名前が空の親マップを探しに行き、あらゆるイベントに対して `None` を返していた。影響を受けていたのは3箇所である。検知ごとの日付・初回/最終イベント時刻の集計、相関エンジンのベースルールのマッチング（タイムスタンプを持たないベースルールのヒットは相関の時間窓に入れられないため、相関が成立することが一度もなく、同梱の相関ルール5本がすべて死んでいた。5分以内に同一IPからコンソールログインに4回失敗しても `Many Failed Logins` の検知は0件だった）、そして相関検知の日付集計である。3箇所とも、`Timestamp` カラムの描画と同じ「存在する最初の候補を採用する」ルールを適用する共通ヘルパー経由で解決するようにしたため、表示される値と分析に使われる値が食い違うことがなくなった。タイムスタンプは RFC 3339、またはUTCとみなす naive な日時としてパースする（`-l, --localtime` の描画と同じ扱い）。 (@fukusuket)
